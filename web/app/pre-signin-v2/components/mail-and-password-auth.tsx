...
import { login, getToken } from '@/service/common'
...
const parseQueryString = (queryString: any) => {
  ...
  for (const pair of pairs) {
    ...
    if (key) {
      ...
      let decodedValue: any = decodeURIComponent(value || '')
...
export default function MailAndPasswordAuth({ isInvite, isEmailSetup, allowRegistration }: MailAndPasswordAuthProps) {
  ...
  const handleEmailPasswordLogin = async (email: string, password: string) => {
    ...
    try {
      ...
      if (res.result === 'success') {
        ...
        router.replace(`/${parseQueryString(window.location.search).path}`)
      }
  ...
  const getUserInfo = async () => {
    const res: any = await getToken({
      url: '/apps/validate-openhydra-token',
      params: {
        token: parseQueryString(window.location.search).mingboToken,
      },
    })
    await handleEmailPasswordLogin(res.difyUserName, res.difyUserPassword)
  }
  useEffect(() => {
    getUserInfo()
  }, [])
